...
extension MenuTileView {
    private func style() {
        ...
        bodyHeaderLabel.text = "Hungry?"
        bodyHeaderLabel.textColor = .white
        
        bodyLabel.translatesAutoresizingMaskIntoConstraints = false
        bodyLabel.font = .systemFont(ofSize: 17, weight: .heavy)
        bodyLabel.text = "Let's fix that"
        bodyLabel.textColor = .white
        
        button.translatesAutoresizingMaskIntoConstraints = false
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Start an Order", for: .normal)
        button.titleLabel?.font = UIFont.preferredFont(forTextStyle: .footnote, compatibleWith: .current)
        button.backgroundColor = UIColor(named: "macdonaldsColor")
